...
$('<link rel="stylesheet" type="text/css" href="//'+Url_1+'utils.imimg.com/im_msg/css/im_msg.css?v=13" >').appendTo("head");
    ...
    if(v4iilex_cookie != undefined && v4iilex_cookie != "" && utype!= undefined && (utype == 'N' || utype == 'F')){
        ...
        if(iso == 'IN'){
            ...
            if (typeof(isImloginV1Open) !== "undefined" && isImloginV1Open == 1) {
                ...
                $.ajaxSetup({cache:!0});
                $.getScript("//"+UrlPri+"utils.imimg.com/header/js/imloginv1-v2.js", function(){
                    fullLoginForm('','','','MSG_Head');
                });
            }
...
function get_id(contact_glid,buyer_name,type) {
    var webAddress=location.hostname;
    var url = "https://"+Url_1+"my.indiamart.com/enquiry/messagecentre/?msg_widget=1&sup_glid="+btoa(contact_glid);
...
function contact_list_ui(response){
    ...
    if(Code2 == 200 && stat2 == "success"){
        ...
        for (var i = 0; i < result_arr.length; i++) {
            ...
            last_message        = last_message.replace('>', '&gt;');
            last_message        = last_message.length >= 35 ? last_message.substr(0, 35) + "..." : last_message;
            ...
            if(c_name != '' || c_mobile != ''){
                ...
                html = html + '</div><div class="chat_wrd_elip chat_contname chat_nm_mob">';
                    ...
                    var add_bold_cls = '';
                    if(c_company == '' && c_city == '' && c_country != '' && c_country.toLowerCase() == 'india'/* && c_state == ''*/)
                        add_bold_cls ='font-size:14px; line-height:16px;color: #000000;';

                html = html + '<span id="c_name" style="'+add_bold_cls+'">'+c_name+'</span>';
                    ...
                    if(product != ''){
                        html = html + '</div><p class="clr wrd_elip chat_prod-p-cls"><span class="chat_prd_icn_green"> </span><span class="chat_prdct_dsply">'+product+'</span></p>';
                    } else{
                        html = html + '</div><p class="clr wrd_elip"></p>';
                        lst_snpt_cls = 'chat_no-prdct-snippet';
                    }
                    
                html = html + '<p class="'+lst_snpt_cls+'chat_wrd_elip">';
